...
describe 'acli', :type => :class do
  ...
  it { should contain_exec('extract_acli').with(:command => '/usr/bin/unzip /tmp/atlassian-cli-3.8.0-distribution.zip -d /opt/') }
  ...
  context "set and version source" do
    ...
    it { should contain_exec('extract_acli').with(:command => '/usr/bin/unzip /tmp/atlassian-cli-3.0.0-distribution.zip -d /opt/') }
